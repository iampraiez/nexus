...
export async function GET(request: NextRequest) {
  try {
    ...
    const versionStatsAgg = await db
      .collection("events")
      .aggregate([
    ...
    const versionStats = versionStatsAgg.map((v) => ({
      ...
      count: v.count,
      percentage:
        totalVersionedEvents > 0
          ? parseFloat(((v.count / totalVersionedEvents) * 100).toFixed(1))
          : 0,
    }));
    ...
    return createSuccessResponse({
      ...
      healthData: healthData.map((d) => ({
        date: d._id,
        deliveryRate:
          d.total > 0 ? parseFloat((((d.total - d.errors) / d.total) * 100).toFixed(1)) : 100,
